...
<script>
   ...
   let brandcate_dls_1_0_0_url = '<?php echo $path ?>';
...
const brandCard = (data) => {
    let name = data.name;
    (name == 'All') ? name = 'Tất cả' : name;
    (name == 'Tiling') ? name = 'Ánh sáng' : name;
    (name == 'Door') ? name = 'Hoàn thiện' : name;
    (name == 'Kitchen') ? name = 'Bếp' : name;
    (name == 'Bathroom') ? name = 'Nhà tắm' : name;
    (name == 'Furniture') ? name = 'Đồ nội thất' : name;
   
    return `
        <div class="brandcate_dls_1_0_0__item">
            <a href="${data.link}"><img src="${data.pic}" alt="${name}"></a>
        </div>
    `;
};
...
const brandNav = (name, link, index) => {
    ...
    let nameView = name;
    (nameView == 'All') ? nameView = 'Tất cả' : nameView;
    (nameView == 'Tiling') ? nameView = 'Ánh sáng' : nameView;
    (nameView == 'Door') ? nameView = 'Hoàn thiện' : nameView;
    (nameView == 'Kitchen') ? nameView = 'Bếp' : name;
    (nameView == 'Bathroom') ? nameView = 'Nhà tắm' : name;
    (nameView == 'Furniture') ? nameView = 'Đồ nội thất' : name;

    return `
        <li class="${active}" onclick="cateFilter('${name}', ${index})">
            <img src="${link}" alt="${nameView}" /> 
            <p>${nameView}</p>
        </li>
    `;
};
